...
def cli(
    ...
    # assume true for this application. we're not interested in card backs.
    ONLY_FRONTS = True
    # HACK: Assumes we're repeatedly testing one card.
    #       Proper impl would be to query paper/card sizes, borderless, etc, but that's non-trivial.
    #       We're going to assume borderless + letter + mtg.
    NUM_CARDS = 9

    for i in range(NUM_CARDS):
        skip = sorted(set(range(NUM_CARDS)) - {i})
        output_path = os.path.join(output_directory, f'game{i}.pdf')
        generate_pdf(
            front_dir_path,
            back_dir_path,
            double_sided_dir_path,
            ...
            # only show the label & reg marks on the first attempt. others are overlaid onto it.
            show_label=i == 0,
            show_registration=i == 0,
        )
